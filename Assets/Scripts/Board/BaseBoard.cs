...
public class BaseBoard : MonoBehaviour
{
    ...
    public Camera UI_CAMERA
    {
        get
        {
            if (UICamera == null)
            {
                UICamera = NGUITools.FindCameraForLayer(LayerMask.NameToLayer("UI"));   // NGUI카메라 반환
            }
            return UICamera;
        }
    }
    ...
    public virtual void UpdateBaord()
    {
        ...
        // 2D UI를 3D월드 좌표로 보내기 ( 월드에 있는 위치값을 뷰포트로 보냈다가 다시 뷰포트의 위치값을 월드로 보냄 )
