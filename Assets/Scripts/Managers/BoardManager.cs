...
public class BoardManager : MonoSingleton<BoardManager>
{
    ...
    private void Update()
    {
        ...
        {
            IList<BaseBoard> listBoard = pair.Value;

            for(int i=0;i<listBoard.Count;i++)
            {
                listBoard[i].UpdateBaord();
    ...
    public void ShowBoard(BaseObject keyObject, bool bEnabel = true)
    {
        if (DicBoard.ContainsKey(keyObject) == false)
            return;
        ...
        for (int i = 0; i < listBoard.Count; i++)
        {
            if (listBoard[i].gameObject.activeSelf != bEnabel)
                listBoard[i].gameObject.SetActive(bEnabel);
        }
    }
